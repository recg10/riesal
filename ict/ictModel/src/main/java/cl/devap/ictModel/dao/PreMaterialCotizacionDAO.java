...
public class PreMaterialCotizacionDAO {
	...
	public List<PreMaterialCotizacion> findAll() {		
		try {
			Query query = manager.createQuery("SELECT preMaterialCotizacion FROM PreMaterialCotizacion AS preMaterialCotizacion ORDER BY preMaterialCotizacion.descripcion ASC");
	        return query.getResultList();
		} catch (Exception ex) {

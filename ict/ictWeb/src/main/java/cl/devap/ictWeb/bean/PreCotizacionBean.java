...
import cl.devap.ict.enums.Parametro;
...
//			preCotizacionDTO.setMontoUnidadCotizacion(valorUnidadCotizacion.intValue());
	...
	public void finalizar(ActionEvent event) {
		...
		try {
	        ...
	        if(save) {
	            message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Success", "Registro actualizado con exito" );  
	            List<ParametroDTO> list = parametroService.findById(new Long(Parametro.MAIL.ordinal()));
	            String mail="rodrigo.e.castro.gonzalez@gmail.com";
	            if (list!=null && list.size()>0){
	            	mail = list.get(0).getTexto();
	            }
	            mailService.enviarMail(mail, "Cotizacion Generada: "+preCotizacionDTO.getIdCotizacion(), "Trabajo: "+preCotizacionDTO.getTrabajo()+"");
	        } else {
	            message = new FacesMessage(FacesMessage.SEVERITY_WARN, "Error", "Error al actualizar");
	        }         
	        FacesContext.getCurrentInstance().addMessage(null, message);        
